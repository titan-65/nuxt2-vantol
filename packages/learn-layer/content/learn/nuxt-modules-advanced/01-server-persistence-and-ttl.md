...
No timers. Eviction happens on read and on write, which means no background interval to leak and no work done while nobody's looking. The array is fine here precisely because the TTL is uniform — insertion order _is_ expiry order, so eviction only ever shifts from the front.
