...
The root juggles gathering _and_ deep research in one context. Long investigations compete for window space and can't easily run in parallel.
...
Use the github**\* and linear**\* tools to pull specifics. Do not pad. If the
...
3. Prompt the root: _"Read recent GitHub PRs, then delegate a deep-dive on the top one to the researcher."_ Confirm it calls `researcher` and folds the result back.
...
- **No parent history.** The child sees only what you pack into `message`. The root must hand it the item's id, URL, and any context — not _"look into that PR we discussed."_
