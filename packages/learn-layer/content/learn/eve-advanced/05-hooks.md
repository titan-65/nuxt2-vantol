...
Hooks are authored extension points for the runtime event stream. A hook subscribes to stream events and runs side effects _after_ each event is durably recorded — audit logging, metrics, alerting, or persisting every session and message to your own database. The slug is the path-relative basename: `agent/hooks/audit.ts` → `"audit"`.
...
Hooks always run _after_ the event is durably recorded, so a thrown hook leaves the stream consistent. A thrown hook propagates as `turn.failed`; a hook on a failure-cascade event that also throws escalates to `session.failed`.
