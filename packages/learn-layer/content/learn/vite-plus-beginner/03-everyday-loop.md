...
These are the same four commands in _every_ Vite+ project. Move between repos and teams and you already know the loop — no reading each project's `package.json` scripts to figure out what "the test command" is this time.
...
- `vp build` builds **apps**. Building a _library_ is a different command (`vp pack`) — that's an Advanced-series topic.
