...
- **Prune prod deps in a _separate_ stage.** Running `vp install --prod` after a full install in the same stage won't remove the already-installed devDependencies — the whole `vite-plus` toolchain would leak into your runtime image.
