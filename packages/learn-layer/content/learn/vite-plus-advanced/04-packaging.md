...
`vp build` builds _apps_; `vp pack` builds _libraries and binaries_, powered by [tsdown](https://tsdown.dev/) (on Rolldown). Out of the box it handles declaration files, multiple output formats, source maps, minification, and CSS bundling — configured in the `pack` block of `vite.config.ts`.
...
Keep it all in `vite.config.ts` (Vite+ recommends _against_ a separate `tsdown.config.ts`):
