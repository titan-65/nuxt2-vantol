...
- **Signature verification:** installing from official `nodejs.org` downloads the PGP-signed `SHASUMS256.txt.asc` and verifies it against bundled release keys _before_ trusting any checksum; the archive's SHA-256 is verified afterward. An invalid signature is a hard error.
