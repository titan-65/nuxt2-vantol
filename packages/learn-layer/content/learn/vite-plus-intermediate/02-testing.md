...
- **`vp test` ≠ `vp run test`.** `vp test` is the built-in Vitest command; `vp run test` runs a `test` _script_ from `package.json`. (Next lesson makes this distinction concrete.)
