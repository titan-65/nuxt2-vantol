...
describe("validateEnv", () => {
  ...
  it("throws when required var is missing", () => {
    expect(() => validateEnv({ API_URL: { type: "url", required: true } }, {})).toThrow(
  ...
  it("uses default when var is empty string", () => {
    const result = validateEnv({ PORT: { type: "number", default: 8080 } }, { PORT: "" });
    expect(result.PORT).toBe(8080);
  });

  it("returns undefined for optional vars with no default", () => {
    const result = validateEnv({ DEBUG: { type: "string", required: false } }, {});
  ...
  it("rejects invalid URL", () => {
    expect(() =>
      validateEnv({ API_URL: { type: "url", required: true } }, { API_URL: "not-a-url" }),
    ).toThrow("API_URL must be a valid URL");
  });

  it("validates number type", () => {
    const result = validateEnv({ PORT: { type: "number", required: true } }, { PORT: "3000" });
    expect(result.PORT).toBe(3000);
  });

  it("rejects invalid number", () => {
    expect(() =>
      validateEnv({ PORT: { type: "number", required: true } }, { PORT: "abc" }),
  ...
  it("validates boolean type", () => {
    const result = validateEnv({ DEBUG: { type: "boolean", required: true } }, { DEBUG: "true" });
    expect(result.DEBUG).toBe(true);
  });

  it("parses boolean 1 as true", () => {
    const result = validateEnv({ DEBUG: { type: "boolean", required: true } }, { DEBUG: "1" });
    expect(result.DEBUG).toBe(true);
  });

  it("parses boolean false", () => {
    const result = validateEnv({ DEBUG: { type: "boolean", required: true } }, { DEBUG: "false" });
    expect(result.DEBUG).toBe(false);
  });

  it("rejects invalid boolean", () => {
    expect(() =>
      validateEnv({ DEBUG: { type: "boolean", required: true } }, { DEBUG: "yes" }),
  ...
  it("defaults to required when required is omitted and no default", () => {
    expect(() => validateEnv({ API_KEY: { type: "string" } }, {})).toThrow(
      "API_KEY is required but missing",
    );
  });

  it("treats required:false explicitly as optional", () => {
    const result = validateEnv({ OPTIONAL: { type: "string", required: false } }, {});
    expect(result.OPTIONAL).toBeUndefined();
  });

  it("treats field with default as not required", () => {
    const result = validateEnv({ PORT: { type: "number", default: 8080 } }, {});
    expect(result.PORT).toBe(8080);
  });
});
