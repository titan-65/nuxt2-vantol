    ...
    <div class="max-w-5xl mx-auto px-6">
      <div v-if="author" class="text-center mb-12 border border-white/10 bg-[#111] rounded-xl p-10 max-w-4xl mx-auto">
        <div class="relative inline-block mb-6">
          ...
          />
          <div class="absolute -bottom-2 -right-2 w-8 h-8 bg-[#f5c542] text-black rounded-full flex items-center justify-center border-4 border-[#111]">
        ...
        <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500 mb-2">Author Profile</p>
        <h1 class="text-4xl md:text-5xl font-semibold tracking-tight leading-tight mb-4">
          {{ author.name }}
        </h1>
        <div class="flex items-center justify-center gap-4 text-xs text-zinc-500 uppercase tracking-widest">
          <span>{{ posts?.length || 0 }} Articles</span>
          <span v-if="author.website" class="w-1 h-1 bg-zinc-700 rounded-full"></span>
          <a v-if="author.website" :href="`https://${author.website}`" target="_blank" class="hover:text-[#f5c542] transition-colors">
            {{ author.website }}
          </a>
        </div>
      </div>
      ...
      <div v-else class="text-center py-20 border border-dashed border-white/10 bg-[#111] rounded-xl max-w-4xl mx-auto">
        <p class="text-zinc-500 text-sm uppercase tracking-widest">No posts found for this author</p>
        <NuxtLink
          to="/blog"
