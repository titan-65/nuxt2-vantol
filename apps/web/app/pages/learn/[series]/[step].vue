...
useSeoMeta({
  title: () => (step.value?.title ? `${step.value.title} | Learn | VantolBennett` : 'Learn | VantolBennett'),
      ...
      <div class="lg:flex gap-12">
        ...
        <aside class="hidden lg:block w-64 shrink-0">
          <div class="sticky top-24 border border-white/10 bg-[#111] rounded-xl p-6">
            <NuxtLink :to="seriesPath" class="block text-sm font-semibold text-white hover:text-[#f5c542] transition-colors mb-1">
              {{ landing?.title || 'Series' }}
            </NuxtLink>
            <p v-if="landing?.nuxtVersion" class="text-[10px] uppercase tracking-widest text-zinc-600 mb-4">Nuxt v{{ landing.nuxtVersion }}</p>
            <ol class="space-y-1">
              <li v-for="(s, i) in siblings" :key="s.path">
                <NuxtLink
                  :to="s.path"
                  class="flex items-center gap-3 px-2 py-1.5 rounded-lg text-xs transition-colors"
                  :class="s.path === route.path ? 'bg-white/10 text-white' : 'text-zinc-500 hover:text-white hover:bg-white/5'"
                >
                  <ClientOnly>
                    <span
                      class="shrink-0 w-5 h-5 rounded-full border flex items-center justify-center text-[10px] font-bold"
                      :class="isComplete(s.path) ? 'bg-[#f5c542] border-[#f5c542] text-black' : 'border-white/20'"
                    >
                      <svg v-if="isComplete(s.path)" class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" />
                      </svg>
                      <template v-else>{{ i + 1 }}</template>
                    </span>
                    <template #fallback>
                      <span class="shrink-0 w-5 h-5 rounded-full border border-white/20 flex items-center justify-center text-[10px] font-bold">{{ i + 1 }}</span>
                    </template>
                  </ClientOnly>
          ...
          <article class="bg-[#111] border border-white/10 rounded-xl p-8 md:p-12">
            <header class="mb-10 border-b border-white/10 pb-8">
              <div class="flex flex-wrap items-center gap-3 mb-6">
                <NuxtLink :to="seriesPath" class="px-2.5 py-1 bg-[#f5c542] text-black text-[10px] font-bold uppercase tracking-widest rounded-md hover:bg-[#e0b13a] transition-colors">
              ...
              <h1 class="text-3xl md:text-4xl font-semibold tracking-tight mb-6 leading-tight">{{ step.title }}</h1>
            ...
            <!-- Content -->
            <div class="prose prose-invert max-w-none font-light prose-headings:font-semibold prose-headings:tracking-tight prose-a:text-[#f5c542] prose-a:no-underline hover:prose-a:underline prose-code:text-[#f5c542] prose-pre:bg-zinc-900 prose-pre:border prose-pre:border-white/10">
            ...
            <div class="mt-12 pt-8 border-t border-white/10">
              <ClientOnly>
                ...
                >
                  ...
                  <span v-if="isComplete(route.path)">Completed{{ next ? ' — next step' : '' }}</span>
