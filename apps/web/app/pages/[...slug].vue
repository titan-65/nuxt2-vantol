...
<template>
  <main class="min-h-screen bg-[#0a0a0a] text-white font-sans">
    <div class="max-w-[1088px] mx-auto px-6 py-12">
      <div
        v-if="page"
        class="bg-[#111] border border-white/10 rounded-xl p-8 md:p-12 max-w-4xl mx-auto"
      >
        <h1 class="text-4xl font-semibold tracking-tight mb-8">{{ page.title }}</h1>
        <div
          class="prose prose-invert max-w-none font-light prose-headings:font-semibold prose-headings:tracking-tight prose-a:text-[#f5c542] prose-strong:text-white"
        >
      ...
      <div
        v-else
        class="text-center py-20 border border-white/10 bg-[#111] rounded-xl max-w-4xl mx-auto"
      >
        ...
        <NuxtLink
          to="/"
          class="inline-block border border-white/10 px-6 py-3 text-xs font-bold uppercase hover:bg-white/5 hover:border-white/20 transition-all rounded-lg text-zinc-300"
        >
          Back Home
        </NuxtLink>
      </div>
