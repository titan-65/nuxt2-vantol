    ...
    <div class="grid lg:grid-cols-2 gap-12 lg:gap-20 items-start">
      ...
      <div>
        ...
        <div class="flex flex-wrap gap-3 mb-8">
          <span
            v-if="item.format"
            class="inline-block px-2.5 py-1 text-xs font-mono bg-[#f5c542] text-black rounded-md uppercase font-bold"
          >
            {{ item.format }}
          </span>
          <span
            v-if="item.pages"
            class="inline-block px-2.5 py-1 text-xs font-mono border border-white/10 text-zinc-400 rounded-md uppercase"
          >
            {{ item.pages }} pages
          </span>
          <span
    ...
    <div v-if="item.toc && item.toc.length > 0">
      ...
      <div class="bg-[#111] border border-white/10 rounded-xl p-8">
        <ol class="space-y-3">
          ...
          >
            <span class="text-xs font-bold text-zinc-600 mt-0.5 w-6 text-right">{{
